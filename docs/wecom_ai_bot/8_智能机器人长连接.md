---
doc_name: 智能机器人长连接
source_url: https://developer.work.weixin.qq.com/document/path/101463
path_id: 101463
doc_id: 60904
official_updated_at: 2026-05-25
...
| [视频消息](#57141/视频消息) | video | 用户发送的视频，仅支持单聊 |
...
> 模板卡片、markdown、文件消息、语音消息、图片消息和视频消息的消息格式参考[支持的消息类型](#消息类型格式说明)

**频率限制**：收到消息回调后，24 小时内可以往该会话回复消息。 无论是回复还是[主动推送消息](#主动推送消息)，总共给某个会话发消息的限制为 30 条/分钟，1000 条/小时。
...
sequenceDiagram
    ...
    Note over Dev,User: 10分钟超时限制：从首次发送开始计时
```
...
| body | object | 是 | 消息内容，详见[回复用户消息](#59068/回复用户消息)中的流式消息和模块卡片消息</br>不支持流式消息+模板卡片的组合消息|
...
**频率限制**：
无论是[回复](#回复普通消息)还是主动推送消息，总共给某个会话发消息的限制为 30 条/分钟，1000 条/小时。
...
| 消息类型 | msgtype | 说明 |
| --- | --- | --- |
| 模板卡片 | template_card | 模板卡片消息，详见[模板卡片](#60904/模板卡片消息) |
| Markdown | markdown | Markdown 格式消息，详见[markdown](#60904/markdown消息) |
|文件消息|file|普通文件消息，详见[文件消息](#60904/文件消息) |
|图片消息|image|图片消息，详见[图片消息](#60904/图片消息) |
|语音消息|voice|语音消息，详见[语音消息](#60904/语音消息) |
|视频消息|video|视频消息，详见[视频消息](#60904/视频消息) |
...
>content字段支持常见的[markdown格式](#14404/markdown-v2类型)
...
|template_card.feedback.id|String|否|若字段不为空值，回复的消息被用户反馈时候会触发[回调事件](#59058/用户反馈事件)。有效长度为 256 字节以内，必须是 utf-8 编码。|
...
**参数说明：**
  ...
  | media_id | 是 | 文件id，可以调用[上传临时素材](#60904/上传临时素材)接口获取 |
...
**请求参数：**
  ...
  | media_id | 是 | 图片媒体文件id，可以调用[上传临时素材](#60904/上传临时素材)接口获取 |
...
**参数说明：**
  ...
  | media_id | 是 | 语音文件id，可以调用[上传临时素材](#60904/上传临时素材)接口获取 |
...
**参数说明：**
  ...
  | media_id | 是 | 视频媒体文件id，可以调用[上传临时素材](#60904/上传临时素材)接口获取 |
